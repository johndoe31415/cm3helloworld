...
class PinmapGenerator(object):
		...
		if attr_str is not None:
			for attr in attr_str.split(","):
				...
				if key not in self._KNOWN_ATTRS:
					raise ParseException("Unknown attribute key '%s' given (only %s are known)." % (key, ", ".join(sorted(self._KNOWN_ATTRS))))
		return result
	...
	def _write_header(self, f):
		...
		print("#include <stm32f10x_gpio.h>", file = f)
		print("#else", file = f)
		...
		for pins in sorted(self._pins.values()):
			for pin in sorted(pins):
				...
				self._print_cols("#define %s_set_HIGH()" % (pin.name), "%s_GPIO->BSRR = %s_Pin" % (pin.name, pin.name), file = f)
				self._print_cols("#define %s_set_LOW()" % (pin.name), "%s_GPIO->BRR = %s_Pin" % (pin.name, pin.name), file = f)
		...
		for (group_name, group_members) in self._groups.items():
			...
			for (port, pins) in sorted(active_low.items()):
				active_commands.append("GPIO%s->BRR = %s" % (port, " | ".join("%s_Pin" % (pin.name) for pin in sorted(pins))))
				inactive_commands.append("GPIO%s->BSRR = %s" % (port, " | ".join("%s_Pin" % (pin.name) for pin in sorted(pins))))
			for (port, pins) in sorted(active_high.items()):
				active_commands.append("GPIO%s->BSRR = %s" % (port, " | ".join("%s_Pin" % (pin.name) for pin in sorted(pins))))
				inactive_commands.append("GPIO%s->BRR = %s" % (port, " | ".join("%s_Pin" % (pin.name) for pin in sorted(pins))))
	...
	def _write_source(self, f):
		...
		print("#include <stm32f10x_gpio.h>", file = f)
		print("#include <stm32f10x_rcc.h>", file = f)
		print("#include \"pinmap.h\"", file = f)

		print(file = f)
		print("void pinmap_initialize(void) {", file = f)
		for port in sorted(used_ports):
			print("	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIO%s, ENABLE);" % (port), file = f)
		print(file = f)
...
#		print(file = f)
		...
		for ((port, mode, otype, speed, pullup), pins) in sorted(grouped_pins.items()):
			...
			init_brr = [ "(%s_Pin)" % (pin.name) for pin in initially_low ]

			print("	{	// %d pin(s) on PORT%s with mode %s, otype %s, speed %s MHz and pullup %s" % (len(pins), port, mode, otype, speed, pullup), file = f)
			if len(init_bsrr) > 0:
				print("		GPIO%s->BSRR = %s;" % (port, " | ".join(init_bsrr)), file = f)
			if len(init_brr) > 0:
				print("		GPIO%s->BRR = %s;" % (port, " | ".join(init_brr)), file = f)
